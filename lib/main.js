...
var test = function()
{
  ...
  ctx.textAlign = "right";
  ctx.fillStyle = "black";
  ctx.font = `${3*font/2}px Arial`;
  ctx.fillText("Click on the", w/2-image_size/4, 30);

  ctx.drawImage(imgs[i], w/2 + image_size/4,0,image_size, image_size);
  testFlag = 1;
}
...
$(document).ready(function() {
  ...
  $('#board').click(function(e){
    ...
    else if(testFlag == -1)
    {
      ...
      if(startFlag == 2)
      {
        ctx.fillText("Try to keep track of their locations to the best of your ability.", w/2, h/2-(font+font/2));
        ctx.fillText("After a few, the animals will freeze and be covered up.",w/2, h/2-(font/2));
        ctx.fillText("You will be asked to click on a target animal.",w/2, h/2+(font/2));
        ctx.fillText("Click to continue...", w/2, h/2 + (font+font/2));
      }
      if(startFlag == 3)
      {
