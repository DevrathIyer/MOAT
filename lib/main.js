...
var setPreferredVelocities = function(simulator) {
  ...
  for (var i = 0; i < simulator.getNumAgents (); ++i) {
    ...
    //v_new = RVOMath.normalize (simulator.getGoal(i).minus(simulator.getAgentPosition(i)));
    //console.log(v_new.x^2 +v_new.y^2);
...
var setupScenario = function(simulator)
{
  ...
  simulator.setAgentDefaults(
      radius*radius*2, // neighbor distance (min = radius * radius)
      30, // max neighbors
      600, // time horizon
      ...
      for (var i=0; i<numAgents; i++) {
        ...
        if(dist <= simulator.getAgentRadius(i))
        {
          ...
          if(target == i)
          {
            ...
            ctx.fillText("Event ".concat(trial).concat("/30 complete! Click 'start' to continue..."), w/2, 30);
